...
const byte c = 0x20, d = 0x21, e = 0x24;
...
uint8_t c_dir0 = 0x80;
uint8_t c_dir1 = 0x03;
uint16_t c_dir = 0x0380;
uint8_t highres = 0x01;
...
//  
  ...
  writeBit(d, LED_Alarm, HIGH);
  writeBit(d, LED_Alarm, LOW);
  
  Wire.requestFrom(c, (byte) 2);
  c_input0 = Wire.read();
  c_input1 = Wire.read(); 
  c_input = (c_input1 << 8) | c_input0;
  //Serial.println(c_input);

  Wire.requestFrom(d, (byte) 2);
  d_input0 = Wire.read();
  d_input1 = Wire.read();
...
{  
  switch(scheduler.poll()){
    case ABFRAGE:
			#ifdef DEBUG
				Serial << "{TIMEPLOT|data|state|T|0}\n";
				Serial << "{MESSAGE|data|ABFRAGE}";
			#endif
			//Serial.println(millis() - time);
      if (interrupt == 1) {
					#ifdef DEBUG
					Serial << "{TIMEPLOT|data|state|T|0.1}\n";
					Serial << "{MESSAGE|data|ABFRAGE.int}";
					#endif
					Wire.beginTransmission(c);
					Wire.write(0x08);
					Wire.endTransmission();
    ...
    //      }
        ...
        if (irrecv.decode(&results)) {
					#ifdef DEBUG
					Serial << "{TIMEPLOT|data|state|T|0.2}\n";
					Serial << "{MESSAGE|data|ABFRAGE.ir}";
					#endif
    ...
    case M_ANSCHALTEN:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|1}\n";
			Serial << "{MESSAGE|data|M_AN1}";
			#endif
      mStatus = SWITCH_ON;
      m_anzuschalten = 0;
    ...
    case M_ANSCHALTEN_PHASE2:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|2}\n";
			Serial << "{MESSAGE|data|M_AN2}";
			#endif
			Serial.println(millis() - time);
			time = millis();
			//noInterrupts();
    ...
    case M_AUSSCHALTEN:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|3}\n";
			Serial << "{MESSAGE|data|M_AUS1}";
			#endif
      mStatus = SWITCH_OFF;
      m_auszuschalten = 0;
			time = millis();
    ...
    case M_AUSSCHALTEN_PHASE2:
		 	#ifdef DEBUG
		 	Serial << "{TIMEPLOT|data|state|T|4}\n";
		 	Serial << "{MESSAGE|data|M_AUS2}";
		 	#endif
		  Serial.println(millis() - time);
    ...
    case EINGANG_WAEHLEN:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|5}\n";
			Serial << "{MESSAGE|data|EING}";
			#endif
    ...
    case B_ANSCHALTEN:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|6}\n";
			Serial << "{MESSAGE|data|B_AN1}";
			#endif
      bStatus = SWITCH_ON;
      b_anzuschalten = 0;
    ...
    case B_ANSCHALTEN_PHASE2:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|7}\n";
			Serial << "{MESSAGE|data|B_AN2}";
			#endif
    ...
    case B_ANSCHALTEN_PHASE3:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|8}\n";
			Serial << "{MESSAGE|data|B_AN3}";
			#endif
    ...
    case B_AUSSCHALTEN:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|9}\n";
			Serial << "{MESSAGE|data|B_AUS1}";
			#endif
      bStatus = SWITCH_OFF;
      b_auszuschalten = 0;
      writeBit(c, bass_Schalter, HIGH);
    ...
    case B_AUSSCHALTEN_PHASE2:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|10}\n";
			Serial << "{MESSAGE|data|B_AUS2}";
			#endif
      writeBit(c, bass_Schalter, LOW);
    ...
    case B_AUSSCHALTEN_PHASE3:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|11}\n";
			Serial << "{MESSAGE|data|B_AUS3}";
			#endif
    ...
    case LAUTSTAERKE:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|18}\n";
			Serial << "{MESSAGE|data|LAUT}";
			#endif
      if (lStatus == M) {
        if (m_kalibrieren == 0) {
    ...
    case FERNBEDIENUNG:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|27}\n";
			Serial << "{MESSAGE|data|FERN}";
			#endif
			//interrupts();
      irrecv.enableIRIn();
      dringlichkeit = 20;
    ...
    case HAUSMEISTER:
			#ifdef DEBUG
			Serial << "{TIMEPLOT|data|state|T|28}\n";
			Serial << "{MESSAGE|data|HAUS}";
			#endif
